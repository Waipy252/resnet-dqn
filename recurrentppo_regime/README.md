# recurrentppo_regime — 日経225トレーディング 強化学習エージェント（レジーム検知版）

日経平均株価（^N225）の時系列データから **買い / 待ち / 売り** を判断する強化学習エージェント。
`recurrentppo_py12_2` をベースに、**HMMレジーム検知**（非定常市場対応）を組み込んだ版。

> ローカルにGPUが無い前提で、**学習は Colab(GPU)・評価/推論はローカル(CPU)** の二段構えを想定（`notebooks/train_colab.ipynb`）。

---

## recurrentppo_py12_2 からの主な変更（レジーム検知, R-1/R-2）

非定常な市場を「レジーム（地合い）の切り替わり」としてモデル化し、検知結果を
エージェントに明示的な文脈として渡す。非定常RLの文脈検知（MBCD系）と
金融のレジームスイッチングモデルの折衷構成。

| 項目 | 内容 |
|---|---|
| **R-1: レジーム検知** | `regime.py`。Open対数リターン＋ローリングσを入力に **ガウシアンHMM**（`N_REGIMES`=3状態）。**ウォークフォワード再学習**（`REGIME_REFIT_EVERY`=63日毎・拡大窓）＋**前向きフィルタ**（filtered事後確率）で **因果的＝未来を見ない**。状態は毎回ボラ昇順に並べ替え、Regime_0=低ボラ〜Regime_2=高ボラの意味を全期間で固定 |
| **R-1: 観測への注入** | 観測 = 正規化済み特徴(17) ＋ **レジーム事後確率(3)** ＋ ポジションone-hot(3) の23次元。確率は[0,1]のままz-scoreしない。LSTMの暗黙の状態推定を明示的な文脈信号で補う |
| **R-2: レジームゲート** | `_eval_regime_ensemble.py`。スコア窓で各モデルの**レジーム別シャープ**を測り、本番は現在の事後確率で加重したスコア最大のモデルに毎日切り替え（MetaTrader-lite）。全モデルのスコアが負の地合いは**FLAT退避**（`REGIME_GATE_FLAT_IF_NEG`） |
| **評価** | `_eval_regime.py` がレジーム別（低ボラ/通常/高ボラ）に年率・シャープを分解し「どの地合いで勝てているか」を出す |
| **互換性** | 観測次元が変わるため **旧 `nikkei_rppo_*` モデルとは非互換**。新モデルは `nikkei_regime_*` プレフィックスで保存。`config.USE_REGIME_OBS=False` で従来観測に戻して比較可能 |
...
評価系スクリプトは **カレントにある `nikkei_*.zip` を自動で拾って** 回す
（観測次元が違う旧 `nikkei_rppo_*` モデルは置かないこと）。
...
uv run python _eval_regime.py          # R-1: レジーム別（低ボラ/通常/高ボラ）に年率・シャープを分解
uv run python _eval_ensemble.py        # 複数モデルの多数決アンサンブルを評価（モデル別にLSTM状態を保持）
uv run python _eval_regime_ensemble.py # R-2: レジームゲート（地合いごとに得意モデルへ毎日切り替え）
```
...
data.py              yfinance/財務省CSV からデータ取得＋テクニカル指標＋レジーム付加（generate_env_data / fetch_latest）
regime.py            R-1: HMMレジーム検知（ウォークフォワード再学習＋前向きフィルタ, 因果的）
main.py              NikkeiEnv（1日1ベクトル観測＋ロバストz正規化＋レジーム事後確率）/ rollout / 学習エントリポイント
...
_eval_regime.py      R-1: レジーム別パフォーマンス分解（どの地合いで勝てているか）
_eval_ensemble.py    多数決アンサンブル評価（モデル別LSTM状態）
_eval_regime_ensemble.py  R-2: レジームゲート・アンサンブル（レジーム別シャープで毎日モデル切替）
...
| `USE_REGIME_OBS` | `True` | R-1: レジーム事後確率を観測に注入（`False` で従来観測） |
| `N_REGIMES` | 3 | HMM状態数（0=低ボラ〜2=高ボラ、ボラ昇順で意味固定） |
| `REGIME_REFIT_EVERY` / `REGIME_MIN_TRAIN` | 63 / 500 | HMM再学習間隔（営業日）/ 最低学習日数（未満は一様分布） |
| `REGIME_VOL_WINDOW` | 21 | HMM入力のローリングσ窓 |
| `REGIME_GATE_FLAT_IF_NEG` | `True` | R-2: 全モデルのスコアが負の地合いで FLAT 退避 |
...
- レジーム検知も因果的: HMMパラメータは「その時点より前のデータ」だけで学習（ウォークフォワード）、
  事後確率は前向きフィルタ（filtered）で計算する。hmmlearn の `predict_proba` は smoothed
  （系列内の未来を参照）なので使っていない。先頭 `REGIME_MIN_TRAIN` 日は一様分布 1/K。
  評価スクリプトの `DATA_START` はこの分だけ余裕を持って前倒ししてある。
- レジームのウォークフォワード再学習で HMM を約100回 fit するため、`generate_env_data` は
  従来より数十秒〜数分余計にかかる（`REGIME_REFIT_EVERY` を増やせば短縮可）。
