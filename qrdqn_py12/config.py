...
# "excess": あんまり。超過リターン（対B&H）。reward = 戦略の対数リターン − 市場(Long固定)の対数リターン（G-3-5）。
#           インフォメーションレシオの即時報酬版。Flatに機会損失が乗り（市場↑の日にFlatだと負）、
#           「B&Hを上回る」ことが直接の目的関数になる。大半がB&H負けの問題に直接効く狙い。
# "excess_dsr": 良い。sharp1.00くらい。差分インフォメーションレシオ（G-3-6）。DSRを超過リターン(戦略−市場)の系列に対して計算する。
#           = "B&H超え"を狙う excess に、DSRのリスク調整＋安定化(warmup/floor/clip)を被せた版。
#           excess単体がノイジーで不安定だったのを、リスク正規化で O(1) スケールの素直な報酬にする。
...
#   1.0 = 純粋なB&H超え（上昇Long保有は報酬0でトレンド乗りが無報酬→信号スパース）。
#   0.5 = 上昇Long保有に +0.5·market の正報酬を残しつつ、様子見/逆張りは相対的に損（推奨）。
#   0.0 = ただの logret（B&H超え圧力なし）。
