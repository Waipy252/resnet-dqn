...
class NikkeiEnv(gym.Env):
    ...
    - 観測（G-1）: 「window×特徴」の2次元から **1日1ベクトル** に変更。
      ...
      観測 = 正規化済み特徴(17) ＋ レジーム事後確率(K, R-1) ＋ ポジション one-hot(3)。
    - レジーム観測（R-1）: data.py が付加した HMM filtered 事後確率
      Regime_0..K-1 をそのまま観測に追加（確率なので z-score はかけない）。
      「いまどの地合いか」の明示的な文脈信号で、LSTM の暗黙の状態推定を補う。
    ...
    def _precompute_features(self):
        ...
        # R-1: レジーム事後確率は [0,1] の確率のまま追加（z-score しない。
        # 既に正規化済みで、「低ボラ〜高ボラ」の意味を歪めないため）。
        self.regime_cols = (
            [f"Regime_{k}" for k in range(config.N_REGIMES)]
            if config.USE_REGIME_OBS and "Regime_0" in self.df.columns
            else []
        )
        if self.regime_cols:
            probs = self.df[self.regime_cols].to_numpy(dtype=np.float32)
            out = np.concatenate([out, probs], axis=1)
        return out
