...
test_data = generate_env_data(
    start, end, ticker=config.TICKER, manual_data=manual_data
)  # 日経平均
window_size = config.WINDOW_SIZE

# バックテスト用（評価用）環境：A-4 で学習と同じコスト・リスク制限を共有
test_env = NikkeiEnv(
    test_data,
...
try:
    # A-5: 評価範囲を学習総ステップ数（35万）に合わせる
    for i in range(200000, config.TOTAL_TIMESTEPS + 1, 10000):
        print(f"## Step {i}")
        ...
        # モデルを安全にロード（存在しないものはスキップ）
        model_path = f"{config.model_name(i)}.zip"
