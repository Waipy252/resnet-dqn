...
# "dsr_pnl": DSR + 実損益（対数リターン）の加重和（G-3-3）。
#            DSRだけだと「ボラ低減＝様子見」に最適化が傾きリターンが伸びないため、
#            実損益を直接報酬に混ぜてリターン方向のインセンティブを与える。
REWARD_TYPE = "dsr_pnl"
...
# dsr_pnl 用: reward = DSR + PNL_WEIGHT * step_log_return。
# 日次対数リターン std≈0.01・DSRは[-1,1]なので、±1%日が±(PNL_WEIGHT*0.01)寄与。
# 既定50: ±1%日が±0.5の寄与 → DSRと同オーダー。大きいほどリターン重視（≒logret寄り）。
PNL_WEIGHT = 50.0
