...
recurrentppo_py12_2 からの主な変更（本プロジェクト recurrentppo_regime）:
- R-1: HMMレジーム検知（regime.py）。レジーム事後確率を観測に注入。
- R-2: レジーム条件付きアンサンブル評価（_eval_regime_ensemble.py）。
"""
...
# ── レジーム検知（R-1: regime.py）────────────────
# ガウシアンHMM（ウォークフォワード再学習＋前向きフィルタ）で市場レジームを
# 因果的に推定し、事後確率 Regime_0..K-1 を観測に注入する。
# Regime_0=低ボラ 〜 Regime_{K-1}=高ボラ（毎回ボラ昇順に並べ替えて意味を固定）。
USE_REGIME_OBS = True      # False で従来観測（レジームなし）に戻して比較できる
N_REGIMES = 3              # 状態数（3: 凪/通常/荒れ。2〜4が定番）
REGIME_VOL_WINDOW = 21     # HMM入力のローリングσ窓（約1ヶ月）
REGIME_REFIT_EVERY = 63    # 何営業日ごとにHMMを再学習するか（約四半期）
REGIME_MIN_TRAIN = 500     # 最初の学習に必要な最低日数（それ以前は一様分布）
REGIME_HMM_ITER = 100      # EMの最大反復
REGIME_SEED = 0            # EM初期化のシード（再現性）

# レジーム条件付きアンサンブル（_eval_regime_ensemble.py）:
# スコア窓で「レジーム別シャープ」をモデルごとに測り、本番では
# 現在のレジーム事後確率で加重したスコアが最大のモデルに従う。
REGIME_GATE_FLAT_IF_NEG = True  # 全モデルのスコアが負のレジームでは FLAT に退避
...
def model_name(steps=None):
    ...
    レジーム観測あり（USE_REGIME_OBS）は観測次元が違い旧 rppo モデルと互換が
    ないため、プレフィックスを nikkei_regime に分けて取り違えを防ぐ。
    """
    algo_tag = "regime" if USE_REGIME_OBS else "rppo"
    base = f"nikkei_{algo_tag}_{TRAIN_START}_{TRAIN_END}"
    if steps is None:
        return base
