...
def generate_env_data(start, end, ticker="JPY=X", manual_data=None, save_csv=False):
    ...
    # -------------------------
    # レジーム検知（R-1）: HMM filtered事後確率 Regime_0..K-1 と RegimeMAP を付加。
    # ウォークフォワード再学習＋前向きフィルタで因果的（行tは寄付き時点の情報のみ）。
    # 学習日数が REGIME_MIN_TRAIN に満たない先頭区間は一様分布（warmup内に収める）。
    # -------------------------
    if config.USE_REGIME_OBS:
        from regime import add_regime_features

        test_data = add_regime_features(test_data)
